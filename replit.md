...
- **New**: Stepped lines (stepped: true, tension: 0) for accurate electrical readings
...
- **New**: PNG export with chart type in filename (puissance/courant)
- **New**: Chart section hidden when no specific device selected
- **New**: Tab switching re-renders chart without re-fetching data (cached)
- **Feature**: Channels shown with distinct colors, filled area, single Y-axis per tab
